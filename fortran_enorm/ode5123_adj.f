...
C adjoint
      DO t=nt-1,2,-1
       ...
       IF (branch .EQ. 0) THEN
        ...
        bub(t) = bub(t) + nu**2*dt*2*wb(t+1)
        bsb(t) = bsb(t) + ns**2*dt*2*wb(t+1)
        wb(t+1) = 0.0
      ENDDO
      write(6,*) 't+1 = ', t+1
      write(16,*) wb(t+1),bub(t+1), bsb(t+1)
      ...
      IF (branch .EQ. 0) THEN
        t = 1
        bsb(t) = bsb(t) + bsb(t+1)
        wb(t) = wb(t) - dt*bsb(t+1)
        bsb(t+1) = 0.0
      ELSE
        t = 1
        bub(t) = bub(t) + bub(t+1)
        wb(t) = wb(t) - dt*bub(t+1)
        bub(t+1) = 0.0
      END IF
      wb(t) = wb(t) + wb(t+1)
      bub(t) = bub(t) + dt*nu**2*wb(t+1)
      bsb(t) = bsb(t) + dt*ns**2*wb(t+1)
      wb(t+1) = 0.0
C      bub(1) = 0.0
C      bsb(1) = 0.0
C      wb(1) = 0.0
