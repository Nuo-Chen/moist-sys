...
use FFTW3
	...
	real,  dimension(128,128) :: in, wrk, out, mask, mask0
	real,  dimension(101) :: tau_z;
	real,  dimension(128,128,101) :: uhat, vhat, what, ThetaRhat, qthat
	real,  dimension(128,128,101) :: u1hat, v1hat, w1hat, ThetaR1hat, qt1hat
	real,  dimension(128,128,101) :: fuhat, fvhat, fwhat, fThetaRhat, fqthat
	real,  dimension(128,128,101) :: fu1hat, fv1hat, fw1hat, fThetaR1hat, fqt1hat
	real,  dimension(128,128,101) :: a_u, a_v, a_w, a_ThetaR, a_qt, a_qv, a_qr
	real,  dimension(128,128,101) :: a_uhat, a_vhat, a_what, a_ThetaRhat, a_qthat
	real,  dimension(128,128,101) :: a_u1hat, a_v1hat, a_w1hat, a_ThetaR1hat, a_qt1hat
	real,  dimension(128,128,101) :: a_fuhat, a_fvhat, a_fwhat, a_fThetaRhat, a_fqthat
	real,  dimension(128,128,101) :: a_fu1hat, a_fv1hat, a_fw1hat, a_fThetaR1hat, a_fqt1hat
	...
	do while ( Ti > 0 )
		...
		do iz = m, 1, -1
			...
			a_qv(:,:,iz) = 0 		 ! set to 0 because qr = 0d0;

			mask(:,:) = qvini(iz) + qt(:,:,iz) .gt. qvs(iz)
			where (mask(:,:))
				a_qt(:,:,iz) = a_qt(:,:,iz) + a_qr(:,:,iz)
				a_qr(:,:,iz) = 0     ! set to 0; in forward qv(:,:,iz) = qt(:,:,iz)-qr(:,:,iz);
			end where 
		end do
		!a_qr = 0
		!a_Theta = 0
		...
		do iz = m,1, -1
			! a_u(:,:,iz) = a_uhat(:,:,iz)
			a_in = a_u(:,:,iz)
			call dfftw_execute_dft_r2c(planf,a_in,a_out)
			call unpadm(nxh,nxph,ny,nyp,nyph,a_out,a_wrk)
			a_uhat(:,:,iz) = a_wrk
		...
		call Thomas(a_rhat,a,b,c,a_phat,nxph,nyp,m-1)
		! Thomas(x,a,b,c,r,nxph,nyp,m); r:inout, x:out
		...
		! initialize the a_f*hat and a_f*1hat
		a_fqthat = 0
		a_fqt1hat = 0
		a_fThetaRhat = 0
		a_fThetaR1hat = 0
		...
		a_fuhat = a_fu1hat + dt*3*e_nu_1uv/4 * a_uhat
		...
		call a_RK_flux(nx, nxh, nxph, ny, nyp, nyph, m, kx, ky, dx, dz, &
				& f_star, g_star, epsbar, l, b_star, nu, vrain, & 
				& u1hat, v1hat, w1hat, ThetaR1hat, qt1hat, u, a_u, v, a_v, w, a_w, ThetaR, a_ThetaR, qt, &
				& a_qt, fu1hat, a_fu1hat, fv1hat, a_fv1hat, fw1hat, a_fw1hat, fThetaR1hat, a_fThetaR1hat, fqt1hat, a_fqt1hat, &
				& rc, mr, ubg, vbg, tau, qvs, qvini, fqvdz, in1, in1b, out1, planr)
		! ad_RK3: u1hat, u, fu1hat, a_fu1hat -> a_u
		...
		do iz = m,1, -1
			...
			a_in = a_v(:,:,iz)
			call dfftw_execute_dft_c2r(planf,a_in,a_out)
			call padm(nxh,nxph,ny,nyp,nyph,a_out,a_wrk)
			a_v1hat(:,:,iz) = a_wrk
		...
		! !******- Possion equation for pressure p -******* . ????? why is this repeated?
		! ! a_Thomas: a_phat -> a_rhat
		! call Thomas(a_rhat,a,b,c,a_phat,nxph,nyp,m-1)
		...
		& u1hat, v1hat, w1hat, ThetaR1hat, qt1hat, u, a_u, v, a_v, w, a_w, ThetaR, a_ThetaR, qt, &
		& a_qt, fu1hat, a_fu1hat, fv1hat, a_fv1hat, fw1hat, a_fw1hat, fThetaR1hat, a_fThetaR1hat, fqt1hat, a_fqt1hat, &
		& rc, mr, ubg, vbg, tau, qvs, qvini, fqvdz, in1, in1b, out1, planr)		!****- RK2  start ********************************
		!************  RK1 end ***********************************
		...
		a_w1hat(:, :, 2:m) =   a_w1hat(:, :, 2:m) + a_rhat(:, :, 1:m-1)/dz
		a_w1hat(:, :, 1:m-1) = a_w1hat(:, :, 1:m-1) - a_rhat(:, :, 1:m-1)/dz
		a_rhat(:, :, 1:m-1) = 0.0 ! done with a_rhat reset to 0
		...
		call a_RK_flux(nx, nxh, nxph, ny, nyp, nyph, m, kx, ky, dx, dz, &
		& f_star, g_star, epsbar, l, b_star, nu, vrain, & 
		& uhat, vhat, what, ThetaRhat, qthat, u, a_u, v, a_v, w, a_w, ThetaR, a_ThetaR, qt, &
		& a_qt, fuhat, a_fuhat, fvhat, a_fvhat, fwhat, a_fwhat, fThetaRhat, a_fThetaRhat, fqthat, a_fqthat, &
		& rc, mr, ubg, vbg, tau, qvs, qvini, fqvdz, in1, in1b, out1, planr)
		!****- RK1  start ********************************
...
! subroutine update_w(uhat, vhat, what, phat, ThetaRhat, qthat, fuhat, fvhat, fwhat, fThetaRhat, fqthat, fu1hat, fv1hat, fw1hat, fThetaR1hat, fqt1hat)
! 	implicit none
